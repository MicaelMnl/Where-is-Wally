...
import UIKit
import Foundation
class HardViewController: UIViewController , UIScrollViewDelegate{
    
    @IBOutlet weak var scrollView: UIScrollView!
    //@IBOutlet var scrollView: UIScrollView!
    let imageView = UIImageView(image: UIImage(named: "bck-h.jpg")!)
    ...
    override func viewDidLoad() {
        ...
        timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(HardViewController.action), userInfo: nil, repeats: true)
        ...
        for i in 0...9 {
            ...
            button.addTarget(self, action: #selector(HardViewController.clicked), for: .touchUpInside)
    ...
    func clicked(sender: UIButton!) {
        ...
        if(!arrayCharlie.checkCharlie()){
            ...
            alert.addAction(UIAlertAction(title: "END", style: .default, handler: { [weak alert] (_) in
                ...
                self.score.setHighHard(score: self.time, message: "\(name):\(self.time)", courrentHightScore : self.score.findHardScore())
